...
공공데이터, 도시/상권, CRM, 리테일 데이터를 다루며 `문제 정의 -> 데이터 구조화 -> 지표 설계/검증 -> 결과물 구현`까지 연결합니다.

- Portfolio: [Signal Archive](https://signal-archive.vercel.app)
- Primary Role: Data Analyst / Spatial Data Analyst
- Secondary Fit: Business Data Analyst / CRM Analytics
- Core Stack: `Python` `SQL` `pandas` `Jupyter` `Tableau` `JavaScript` `GIS`
...
- 단순 요약보다 `무엇을 먼저 봐야 하는가`를 설명하는 문제 정의 감각
- AI-assisted workflow를 활용하되, 문제 정의와 최종 해석은 직접 검토하는 작업 방식

## Contact

- Portfolio Contact: [signal-archive.vercel.app/contact](https://signal-archive.vercel.app/contact)
- GitHub: [dffxonnb-cyber](https://github.com/dffxonnb-cyber)
